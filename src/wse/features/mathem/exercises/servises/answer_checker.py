...
class AnswerChecker:
    ...
    def check(self, user_answer: IAnswer, storage: ITaskStorage) -> None:
        ...
        try:
            correct_answer = storage.answer.text
        except AttributeError:
            logger.error('Attempted to check answer with no stored task')
            self._is_correct = None
        else:
            self._is_correct = user_answer.text == correct_answer

    @property
    def result(self) -> Result | None:
        """The representation of user answer check."""
        return (
            Result(self._is_correct) if self._is_correct is not None else None
        )
